...
int main(int argc, char **argv)
{
    ...
    // Initial desired robot state  // The comments near the lines are used to improve last request of point 2b 
    init_jnt_pos[0] = 0.0;
    init_jnt_pos[1] = 1.57;
    init_jnt_pos[2] = -1.57;  //-1.30
    init_jnt_pos[3] = -1.57;
    init_jnt_pos[4] = 1.57;   //1.30
    init_jnt_pos[5] = -1.57;  //-1.20
    init_jnt_pos[6] = 1.57;
    ...
    while (true)
    {
        if (robot_state_available && aruco_pose_available)
        {
            ...
            // Create frame offset rotated by pi around x axis and placed by 0.5 along z axis
            ...
            KDL::Frame base_T_frame_offset = robot.getEEFrame()*frame_offset;

            // Compute errors
            Eigen::Matrix<double,3,1> e_o = computeOrientationError(toEigen(base_T_frame_offset.M), toEigen(robot.getEEFrame().M));
            Eigen::Matrix<double,3,1> e_o_w = computeOrientationError(toEigen(Fi.M), toEigen(robot.getEEFrame().M));
            Eigen::Matrix<double,3,1> e_p = computeLinearError(toEigen(base_T_frame_offset.p),toEigen(robot.getEEFrame().p));  
            ...
            Eigen::Matrix<double,3,3> L_first = (-1/cP_o.norm())*(Eigen::MatrixXd::Identity(3,3)-s*s.transpose());
            L.block(0,0,3,3) = L_first;
